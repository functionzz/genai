// shadcn-file-upload by kwesinavilot, converted into Typescript
...
// Define an interface for File objects with a preview property
interface FileWithPreview extends File {
    preview: string;
}

// Define the prop types for the FileUpload component
...
const FileUpload: React.FC<FileUploadProps> = ({
    ...
    const onDrop = useCallback((acceptedFiles: File[]) => {
        ...
        if (uploadMode === 'single') {
            ...
            // Validate all files
            const errorsArr = newFiles.map(validateFile).filter(Boolean) as string[];
            ...
            if (errorsArr.length === 0) {
                ...
                setInternalErrors(errorsArr[0]); // Display only the first error
            }
        }
